...
def integrate(dry_run: bool) -> None:
    ...
    - Generates .pantheon/quality-config.json with auto-discovered quality commands
    - Installs quality gate hooks (SubagentStop, PreCommit, PhaseGate,
      OrchestratorCodeGate)

    The quality config is generated by analyzing your project structure and discovering
    test, lint, and type-check commands automatically.
    """
    ...
    if dry_run:
        ...
        click.echo("\nWould generate:")
        click.echo("  - .pantheon/quality-config.json (auto-discovered commands)")
        click.echo("\nWould install hooks:")
        ...
        click.echo("  - .pantheon/hooks/orchestrator-code-gate.sh (Write/Edit guard)")
    ...
    click.echo("\nGenerating quality configuration...\n")
    from pantheon.quality.config import generate_quality_config

    try:
        # Look for plan.md in specs/ directories or project root
        plan_path = None

        # Check project root first
        if (cwd / "plan.md").exists():
            plan_path = cwd / "plan.md"
        else:
            # Check for plan.md in specs/ subdirectories
            specs_dir = cwd / "specs"
            if specs_dir.exists():
        ...
        config_path = generate_quality_config(cwd, plan_path)
        ...
        click.echo(f"  Project type: {config.get('project_type', 'unknown')}")
        click.echo(f"  Discovery source: {config.get('discovery_source', 'unknown')}")

        commands = config.get('commands', {})
        if any(commands.values()):
            click.echo("  Commands discovered:")
            for cmd_type, cmd in commands.items():
                if cmd:
                    click.echo(f"    • {cmd_type}: {cmd}")
        else:
            click.echo("  ⚠️  No quality commands discovered")
            click.echo("     Hooks will run but skip validation checks")

    except Exception as e:
        click.echo(f"⚠️  Could not generate quality config: {e}")
        click.echo("   Hooks will have limited functionality")
